...
//	Cmycomplex a(x1, y1), b(x2, y2), c(x3, y3), z, t1, t2;
	...
	Cmycomplex resa = ((y3-y1)*(x2-x1)-(y2-y1)*(x3-x1))/((x3*x3-x1*x1)*(x2-x1)-(x2*x2-x1*x1)*(x3-x1));
	Cmycomplex resb = ((y2-y1)-resa*(x2*x2-x1*x1))/(x2-x1);
	Cmycomplex resc = (y1-resa*(x1*x1))- resb*x1;
	
	printf("%lf %lf\n",resa.getReal(),resa.getImaginary());
	printf("%lf %lf\n",resb.getReal(),resb.getImaginary());
	printf("%lf %lf\n",resc.getReal(),resc.getImaginary());
}
