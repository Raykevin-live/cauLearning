...
class Score {
private:
    vector<int> scores;  // 成绩数组，最大长度N
    int scoresCount;      // 已经存储的成绩数量 

public:
    // 构造函数，初始化scoresCount为0
    Score(int maxSize = N) : scoresCount(0) {}  
    ...
    int queryScore(int xScore) const {
        if((auto iter = find(scores.begin(), scores.end(), xScore))!=scores.end()){
        	return iter-scores.begin();
		}
        return -1;
    }
